<template>
    <div class="container my-5">
        <h2>Hoteles</h2>

        <div class="row">
            <div class="col-md-4 mt-4" v-for="hotel in this.hoteles " v-bind:key="hotel.id">

                <div class="card">
                    <img class="card-img-top" :src="`storage/${hotel.imagen_principal}`" alt="card del restaurant" >
                    <div class="card-body">
                        <h4 class="card-title text-primary"> {{ hotel.nombre }} </h4>
                        <h3 class="card-text fw-light"><span class="fw-bold">Ubicación: </span> {{hotel.direccion}}</h3>
                        <h3 class="card-text fw-light">
                            <span class="fw-bold">Horario:</span>
                                {{hotel.apertura}} - {{hotel.cierre}}
                        </h3>  
                        <router-link :to=" {name:'establecimiento', params: {id:hotel.id}}">
                            <a class="btn btn-primary d-block">Ver Lugar</a>
                        </router-link> 

                    </div>
                </div>
            </div>
        </div>
    </div>
</template>

<script>

export default{

    mounted(){
        axios.get('/api/categorias/hotel')
        .then(response =>{
            this.$store.commit("AGREGAR_HOTEL", response.data);
        })

    },
    computed: {
        hoteles(){
            return this.$store.state.hoteles
        }
    }
}

</script>

